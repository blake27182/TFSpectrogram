...
def get_duration(wav_file):
	with contextlib.closing(wave.open(wav_file,'r')) as f:	# open file so that it closes right after with block
...
def split_wav(wav_file, segment):
	n = 1					# initialize file name iterator to 1
	s = segment * 1000		# store number of milliseconds in segment
	...
	for i in range(int(duration/segment)):	# for number of times segment fits into duration evenly
		...
		newAudio.export('waves/%s.wav' %(n), format="wav")	# write newAudio to a file in waves directory
		print (n, t1, t2)
